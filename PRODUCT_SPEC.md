...
V1 预置四个，不可增删：`🍜 吃什么` `🎉 玩什么` `✈️ 去远方` `💆 休息一下`。

- 为什么要有场景：**决策是有语境的**。中午十二点的决策池里出现「去拉萨」，抽取立刻失效。场景是抽签质量的地基。
- 为什么不可自定义：自定义是「分类整理欲」的入口，而整理是收集的反面。V1.5 只放开一个自定义名额。
- 场景筛选在抽屉里，默认「全部」——P5。
...
**V1 明确不做**：note 字段、自定义场景、预算偏好、导出/导入、深色模式（纸感暖色本身就是基调）。MVP 的敌人不是缺功能，是每个功能 3 倍的维护成本。
...
| 4 | 一个自定义场景 | 给「遛娃」「撸猫」开口子，用数量上限挡住分类整理欲 |
