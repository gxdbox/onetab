...
>
> **v0.3 变更**：自定义场景从「固定 1 个」升级为「可增删多个」，并支持在记一个页当场新建（需求 1）；首页新增「今天吃什么」直达（需求 2-A）；**情侣共册**云同步共享池（需求 2-B）。
> - ~~自定义场景上限一个~~ → **可增删多个**，label ≤ 4 字 + 同名去重，id 形如 `c:电影`；删除时条目改派回预置场景（§3.2、§5.2）
> - 首页新增 **🍜 今天吃什么** 一键直达：锁定 eat 场景直接开抽（§6.2 备注）
> - **💑 情侣共册**（§7.2）：两个 openid 共享同一份快照文档；云函数 `couple` 服务端校验成员身份；口令（6 位）创建/加入；拉取仍走 updatedAt 合并语义 [硬约束 #9]。云是共享快照不是真相，并发推送以最后写入者为准
>
...
- 两个 openid 通过 6 位口令加入同一「共册」，云端只存**一份共享快照**（`onetab_snapshots/{roomId}`）。
- 云函数 `couple`（create / join / leave / myRoom / getSnapshot / putSnapshot）在**服务端校验成员身份**——客户端无法越权读别人的册子。
- 拉取与个人同步同走 `repo.importData()` 的 updatedAt 合并语义【硬约束 #9】：谁的新就留谁的，两边都不丢。
- **已知边界（诚实记录）**：云端是快照不是真相，两人**同时**离线各自改动后并发推送，最后写入者覆盖先写者（该条在下次对端拉取时以本地为准补回）。情侣场景写频率低，可接受；若未来出现丢数据反馈，再升级 putSnapshot 为服务端合并。
- 未填 `SYNC_ENV_ID` 时整个同步层与共册 UI 都不出现——本地唯一真相的铁律不因 V2 松动。
